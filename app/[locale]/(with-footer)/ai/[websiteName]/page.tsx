...
export async function generateMetadata({
  ...
  try {
    ...
    return {
      ...
      description: res.data.content || t('defaultDescription'),
    };
  } catch (error) {
    return {
...
export default async function Page({ params: { websiteName } }: { params: { websiteName: string } }) {
  ...
  try {
    const res = await getWebNavigationDetail(websiteName);
    if (!res || !res.data || !res.data.title) {
      return <div>{t('noDataMessage')}</div>;
    }
    ...
    return (
      <div className='w-full bg-gray-100'>
        <div className='flex flex-col px-6 py-5 lg:h-[323px] lg:flex-row lg:justify-between lg:px-0 lg:py-10'>
          <div className='flex flex-col items-center lg:items-start'>
            <div className='space-y-1 text-balance lg:space-y-3'>
              <h1 className='text-black-900 text-2xl lg:text-5xl'>{data.title}</h1>
              <h2 className='text-black-700 text-xs lg:text-sm'>{data.content}</h2>
            </div>
            <a
              href={data.url}
              target='_blank'
              rel='noreferrer'
              className='flex-center mt-5 min-h-5 w-full gap-1 rounded-[8px] bg-gray-300 p-[10px] text-sm capitalize text-gray-900 hover:opacity-80 lg:mt-auto lg:w-[288px]'
            >
          ...
          >
            <BaseImage
              ...
              className='absolute mt-3 aspect-[466/234] w-full rounded-[16px] border border-gray-400 bg-gray-400 bg-cover lg:mt-0'
            />
        ...
        <div className='mb-5 px-3 lg:px-0'>
          <h2 className='my-5 text-2xl text-gray-700 lg:my-10'>{t('introduction')}</h2>
